...
def get_access_token():
    ...
    response = requests.get("https://oauth.vk.com/authorize?client_id=< айди приложения >&display=page&redirect_uri=https://oauth.vk.com/blank.html&scope=notify,friends,photos,audio,video,stories,pages,status,notes,wall,ads,offline,docs,groups,notifications,stats,email,market&response_type=token&v=5.131&state=123456")
...
def get_user_for_phone(phone):
    ...
    # data = '{"email": "' + email + '", "password": "' + password + '"}'
    request = api + "account.lookupContacts.json?contacts=" + phone + "&service=phone&mycontact=&return_all=0&fields=nickname&v=5.131&access_token=" + access_token
    # request = api + "account.getProfileInfo?user_id=" + user_id + "&v=5.81" + "&access_token=" + access_token
